...
def _attention(Q, K, V, bias1, bias2):
    ...
    assert bias1.is_cuda, "bias1 must be on cuda"
    assert bias2.is_cuda, "bias2 must be on cuda"
    global kernel_
    if kernel_ is None:
...
def attention_bwd(dO, Q, K, V, O, lse, bias1, bias2):
    assert max(Q.shape[-1], V.shape[-1]
    ...
    assert dO.is_cuda, "dO must be on cuda"
    assert Q.is_cuda, "Q must be on cuda"
    assert K.is_cuda, "K must be on cuda"
    assert V.is_cuda, "V must be on cuda"
    assert O.is_cuda, "O must be on cuda"
    global kernel_
    if kernel_ is None:
        kernel_ = EvoformerAttnBuilder().load()
...
class EvoformerFusedAttention(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        dQ, dK, dV, dB1, dB2 = attention_bwd(
            grad_output, q, k, v, o, lse, bias1, bias2)
        if bias1.numel() == 0:
            dB1 = None
        if bias2.numel() == 0:
            dB2 = None
...
def EvoformerAttention(Q, K, V, biases):
    assert len(biases) <= 2
