...
template <class AccessType, class Enable = void> struct atomic_store {};

template <class AccessType>
struct atomic_store<AccessType,
                    typename platform::enable_if<platform::is_same<
                        typename AccessType::Element, half_t>::value>::type> {
...
struct atomic_store<AccessType,
                    typename platform::enable_if<platform::is_same<
                        typename AccessType::Element, float>::value>::type> {
  using Element = typename AccessType::Element;
  static const int kCount = AccessType::kElements;
  ...
  struct Params {
    ...
    Params(TensorCoord const &extent, Layout const &layout_) : layout(layout_) {
      ...
      for (int i = 0; i < Layout::kRank / 2; ++i) {
        ...
        stride_n[i] =
            OffsetBytes<Element>(layout_.stride()[i + Layout::kRank / 2]);
      }
  ...
  PredicatedTileIteratorAffineRankNAtomic(
    ...
    if (Layout::kRank > 2) {
      ...
      for (int c = 0; c < ThreadMap::Iterations::kColumn; ++c) {
        ...
        if (kBigEndian) {
          modes_n = CoordinateDecomposition<Layout::kRank / 2>(
    ...
    for (int cluster = 0; cluster < ThreadMap::Iterations::kCluster;
      ...
      for (int group = 0; group < ThreadMap::Iterations::kGroup; ++group) {
        ...
        for (int row = 0; row < ThreadMap::Iterations::kRow; ++row) {
          ...
          if (Layout::kRank > 2) {
            if (kBigEndian) {
              modes_m = CoordinateDecomposition<Layout::kRank / 2>(
            ...
            if (Layout::kRank > 2) {
              ...
              guard = (coord_m < extent_row_) &&
                      ((thread_start_column_ +
                        ThreadMap::Delta::kColumn * column) < extent_col_);
            }

            atomic_store<AccessType>(
                frag_ptr[frag_row_idx * ThreadMap::Iterations::kColumn +
                         column],
                (void *)(byte_pointer + offset_modes_m + offset_modes_n +
                         byte_offset),
                guard);
      ...
      for (int group = 0; group < ThreadMap::Iterations::kGroup; ++group) {
        ...
        for (int row = 0; row < ThreadMap::Iterations::kRow; ++row) {
          ...
          for (int column = 0; column < ThreadMap::Iterations::kColumn;
            ...
            if (!PermuteD) {
              memory_pointer +=
                  (ThreadMap::Delta::kColumn / kElementsPerAccess);
            }
          }
