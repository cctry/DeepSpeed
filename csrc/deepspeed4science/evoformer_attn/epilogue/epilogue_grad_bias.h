...
struct EpilogueVoltaTensorOpAffineRankN
    : public DefaultEpilogueVoltaTensorOpAffineRankN<Rank, Shape_, WarpMmaTensorOp_,
                                                PartitionsK, OutputOp_,
                                                ElementsPerAccess> {
  using Base = DefaultEpilogueVoltaTensorOpAffineRankN<Rank, Shape_,
                                                  WarpMmaTensorOp_, PartitionsK,
                                                  OutputOp_, ElementsPerAccess>;
  using OutputTileIterator =
      cutlass::epilogue::threadblock::PredicatedTileIteratorAffineRankNAtomic<
...
template <typename Shape_, typename WarpMmaTensorOp_, int PartitionsK,
          typename OutputOp_, int ElementsPerAccess, bool ScatterD = false,
          typename PermuteDLayout = layout::NoPermute>
struct EpilogueVoltaTensorOp
    : public DefaultEpilogueVoltaTensorOp<Shape_, WarpMmaTensorOp_, PartitionsK,
                                     OutputOp_, ElementsPerAccess, ScatterD,
                                     PermuteDLayout> {
  using Base =
      DefaultEpilogueVoltaTensorOp<Shape_, WarpMmaTensorOp_, PartitionsK, OutputOp_,
                                   ElementsPerAccess, ScatterD, PermuteDLayout>;
  using OutputTileIterator =
      cutlass::epilogue::threadblock::PredicatedTileIteratorAtomic<
          typename Base::OutputTileThreadMap, typename Base::ElementOutput,
...
template <typename Arch_, typename Shape_, typename WarpMmaTensorOp_, int PartitionsK,
          typename OutputOp_, int ElementsPerAccess, bool ScatterD = false,
...
struct BiasGradEpilogue {
    using Epilogue = typename cutlass::epilogue::threadblock::EpilogueTensorOp<
          Shape_, WarpMmaTensorOp_, PartitionsK,
          OutputOp_, ElementsPerAccess, ScatterD, PermuteDLayout>::Epilogue;
};

template <typename Shape_, typename WarpMmaTensorOp_, int PartitionsK,
          typename OutputOp_, int ElementsPerAccess, bool ScatterD,
          typename PermuteDLayout>
struct BiasGradEpilogue<cutlass::arch::Sm70, Shape_, WarpMmaTensorOp_, PartitionsK,
                        OutputOp_, ElementsPerAccess, ScatterD, PermuteDLayout> {
    using Epilogue = typename cutlass::epilogue::threadblock::EpilogueVoltaTensorOp<
          Shape_, WarpMmaTensorOp_, PartitionsK,
          OutputOp_, ElementsPerAccess, ScatterD, PermuteDLayout>::Epilogue;
};

template <typename Arch_, int Rank, typename Shape_, typename WarpMmaTensorOp_, int PartitionsK,
          typename OutputOp_, int ElementsPerAccess>
struct BiasGradEpilogueAffineRankN {
    using Epilogue = typename cutlass::epilogue::threadblock::EpilogueTensorOpAffineRankN<
          Rank, Shape_, WarpMmaTensorOp_, PartitionsK,
          OutputOp_, ElementsPerAccess>::Epilogue;
};

template <int Rank, typename Shape_, typename WarpMmaTensorOp_, int PartitionsK,
          typename OutputOp_, int ElementsPerAccess>
struct BiasGradEpilogueAffineRankN<cutlass::arch::Sm70, Rank, Shape_, WarpMmaTensorOp_, PartitionsK,
                        OutputOp_, ElementsPerAccess> {
